...
data Op = BinOp BinOp | In | Out | Jnz | Jz | Halt deriving (Eq,Show)
...
data Mode = Ind | Imm deriving (Eq,Show)
...
rd :: Mode -> Int -> M Int
rd m x = getRel x >>= readMode m

applyOp :: Op -> [Mode] -> M Bool
applyOp Halt _ = return True
applyOp op modes = do
  inps <- sequence$ zipWith rd modes [1..(fst$ struct op)]
  applyWith op inps
  modify (incIP $ len op)
  return False
...
applyWith x y = throwError ("Wrong number of args for "++ show x ++":"++ show y)
